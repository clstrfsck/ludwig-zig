...
const Parser = struct {
    ...
    fn parseRules(self: *Parser, indent: usize) anyerror![]const data.Rule {
        var list: std.ArrayListUnmanaged(data.Rule) = .{};
        while (self.peek()) |line| {
            if (line.indent < indent) break;
            if (line.indent != indent) return error.InvalidIndentation;
...
fn generateSyntaxData(allocator: std.mem.Allocator, input_dir: []const u8) ![]u8 {
    ...
    var output: std.ArrayListUnmanaged(u8) = .{};
    try output.appendSlice(allocator, "// Code generated by ludwig-syntax-gen; DO NOT EDIT.\n");
    try output.appendSlice(allocator, "// Syntax definitions originate from highlight assets adapted from micro.\n");
    try output.appendSlice(allocator, "const data = @import(\"syntax_data_types\");\n\n");
    try output.appendSlice(allocator, "pub const attribution = ");
    try writeZigString(output.writer(allocator), "Syntax definitions originate from highlight assets adapted from micro; keep attribution and licensing explicit.");
    try output.appendSlice(allocator, ";\n");
    try output.appendSlice(allocator, "pub const syntax_files = [_]data.SyntaxFile{\n");
